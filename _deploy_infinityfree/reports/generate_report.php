...
function convert_html_to_pdf($htmlPath, $pdfPath) {
    ...
    if (!$wkhtmltopdf) {
        return ['success' => false, 'error' => 'wkhtmltopdf executable not found.', 'code' => 500];
    }
...
function generate_test_report($baseUrl, $pages, $testedBy, $automationChecks = null) {
    ...
    $summary = [
        'total' => $sumFunctional + $sumSeo + $sumSsl,
    ...
    try {
        ...
        if (file_put_contents($reportHtmlPath, $htmlContent) === false) {
            return ['success' => false, 'error' => 'Unable to write HTML report.', 'code' => 500];
        }

        $pdfResult = convert_html_to_pdf($reportHtmlPath, $reportPdfPath);
        if (!$pdfResult['success']) {
            return ['success' => false, 'error' => $pdfResult['error'] ?? 'PDF creation failed', 'code' => $pdfResult['code'] ?? 500];
        }
    } catch (Exception $e) {
    ...
    return [
        ...
        'view_url' => '/reports/generated/' . basename($reportPdfPath),
        'download_url' => '/reports/generated/' . basename($reportPdfPath),
        'issues_total' => $summary['total'],
        'seo_issues' => $sumSeo,
        'ssl_issues' => $sumSsl,
        'functional_issues' => $sumFunctional,
