...
function run_automation_checks($baseUrl) {
    ...
    $commandArgs = array_map('escapeshellarg', [$script, $config, $baseUrl, $outputPath]);
...
try {
    ...
    $baseUrl = $pages[0]['url'];
    $automationPayload = run_automation_checks($baseUrl);
    ...
    if (!empty($result['success']) && $result['success'] === true) {
        ...
        try {
            if (isset($conn) && $conn instanceof mysqli) {
                ...
                // Extract PDF path (view_url contains the actual generated PDF path)
                $pdf_path = $result['view_url'] ?? '';  // Use PDF path from view_url
                
                // Validate PDF file exists and is valid before saving to database
                if (!empty($pdf_path)) {
                    $relative_path = ltrim($pdf_path, '/');
                    $full_pdf_path = __DIR__ . '/../' . $relative_path;
                    ...
                    // Validate file is a valid PDF (check magic bytes and size)
                    ...
                    send_json(['success' => false, 'error' => 'Report generated but PDF path is missing'], 500);
                }
                
                // Log the test report (schema matches table columns: test_link, execution_date, pdf_path)
                $report_id = $reporter->logTestReport($baseUrl, $pdf_path);
